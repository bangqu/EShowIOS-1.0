...
- (void)setHotkeys:(NSArray *)hotkeys {
    ...
    if ([hotkeys count] <= 0) {
        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, ScreenWidth, 0);
        return;
    }
    ...
    if(hotkeys.count) {
        ...
        CGFloat maxWidth = ScreenWidth - 20.0f;
...
//            currentSize = CGSizeMake(currentSize.width, currentSize.height + 10.0f);
        ...
        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, ScreenWidth, currentHeight);
    }
}

- (CGSize)computeSizeFromString:(NSString *)string {
    
    CGSize maxSize = CGSizeMake(ScreenWidth, 30);
