...
- (void)layoutSubviews {
    ...
    if (_isCaptcha) {
        [self.textField setWidth:(ScreenWidth - 2*kLoginPaddingLeftWidth) - (_isForLoginVC? 90 : 70)];
        _captchaView.hidden = NO;
        [self refreshCaptchaImage];
    }else{
        [self.textField setWidth:(ScreenWidth - 2*kLoginPaddingLeftWidth) - (_isForLoginVC? 20:0)];
...
- (void)setCaptchaImage:(UIImage *)captchaImage{
    __weak typeof(self) weakSelf = self;
    if (!_captchaView) {
        _captchaView = [[UITapImageView alloc] initWithFrame:CGRectMake(ScreenWidth-60-kLoginPaddingLeftWidth, (44-25)/2, 60, 25)];
