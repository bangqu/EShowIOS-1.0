...
#define FTDefaultTextColor          [UIColor colorWithRed:200/255.f green:200/255.f blue:200/255.f alpha:1.f]
...
//        senderRect.origin.y = MAX(64-senderRect.origin.y, senderRect.origin.y);
    ...
    if (arrowDirection == FTPopOverMenuArrowDirectionUp) {
        menuRect = CGRectMake(menuX, (senderRect.origin.y + senderRect.size.height)+15, self.preferedWidth, menuHeight);
        // if too long and is out of screen
        if (menuRect.origin.y + menuRect.size.height > KSCREEN_HEIGHT) {
            menuRect = CGRectMake(menuX, (senderRect.origin.y + senderRect.size.height)+15, self.preferedWidth, KSCREEN_HEIGHT - menuRect.origin.y - FTDefaultMargin);
            shouldAutoScroll = YES;
        }
    }else{
        
        menuRect = CGRectMake(menuX, (senderRect.origin.y - menuHeight)+15, self.preferedWidth, menuHeight);
        // if too long and is out of screen
        if (menuRect.origin.y  < 0) {
            menuRect = CGRectMake(menuX, FTDefaultMargin, self.preferedWidth+15, senderRect.origin.y - FTDefaultMargin);
